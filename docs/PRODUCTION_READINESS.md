...
2. Lifecycle tests: already-dead stop, isolated supervisor tree-kill, repeated stop, identity mismatch.
...
| Windows CI runs `tests/test_lh_process_windows.py` | **Real** | Windows job in `.github/workflows/ci.yml`. Isolated fixtures, including already-dead stop and a tree-kill case. Linux CI does not run this file. This is not `gate_job_object_kill_path_v1` and not a live operator-plant tree-kill |
