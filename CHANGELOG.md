...
- **Single-flight admit:** a failed campaign-lock acquire observes `role_alive` and the claim before it returns `lock_timeout`. Windows run 35916007830 still refused the loser of `test_contended_start_and_recover_admit_one_spawn` with `lock_timeout` after create-time probes moved outside the hold: the 5s budget expired while the winner was alive and the admit was already visible. `lock_timeout` remains only when the role and the claim still look dead. Exactly one spawn, and a refusal of `role_alive` or `claim_held`, stay required. Create-time probes stay outside the exclusive admit hold. This does not move `create_time_fn` out of `_claim_blocks` while the lock is held, and it does not reseal the assurance row.
- **Single-flight admit:** create-time reads stored on a new supervisor claim run after the campaign lock is released. Windows CIM queries were inside that hold, so a second controller could return `lock_timeout` before it observed `role_alive` or `claim_held` (Windows run 35697613441, `test_contended_start_and_recover_admit_one_spawn`). The lock still covers the role check, the claim publish, and spawn. A live claim with no create_time yet still refuses another spawn. The race test still requires exactly one spawn and a refusal of `role_alive` or `claim_held`. This does not reseal the assurance row.
- **stop_old identity refusal:** a Windows liveness probe uses `tasklist` through `subprocess`. A live pid whose command line is not the supervisor still returns `stop_old_refused:identity_mismatch` and does not spawn. Run 35699729432 reported `stop_old_refused:pid_check:AssertionError` when a test stub raised on every `Popen`, including that probe. A regression test forces the `tasklist` path on every OS.
- **Dead-PID test helper:** the helper no longer calls `CloseHandle` on a `Popen` process handle. `Popen` closes that handle on GC; a second close raised `WinError 6` on the Windows job. Liveness uses the process exit code.

- **Stop and relaunch scope** ([README.md](README.md), [FINAL_STATE_BRIEF.md](FINAL_STATE_BRIEF.md), [docs/OPERATIONS.md](docs/OPERATIONS.md)): tree-kill and Job Object stop are **Proved on Windows** only for `gate_job_object_kill_path_v1` (and **Real on Windows** for `tests/test_lh_process_windows.py`). They are **Locked on non-Windows**. Watchdog relaunch stays **Pending**, including on non-Windows. The single-PID SIGTERM branch is not a tree stop. This repo is not a portable control plane.
